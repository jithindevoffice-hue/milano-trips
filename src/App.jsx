...
function TripSearch({ onQuote }) {
  const [destination, setDestination] = useState('');
  const [type, setType] = useState('');
  const [date, setDate] = useState('');
  const navigate = useNavigate();
  return <form className="trip-search" onSubmit={(e) => { e.preventDefault(); navigate(`/destinations?search=${encodeURIComponent(destination)}&category=${encodeURIComponent(type)}`); }}>
    <label><MapPin size={19} /><span>Destination<input value={destination} onChange={e => setDestination(e.target.value)} placeholder="Where to?" aria-label="Destination" /></span></label>
    <label><Compass size={19} /><span>Travel style<select value={type} onChange={e => setType(e.target.value)} aria-label="Travel style"><option value="">Any style</option>{CATEGORIES.slice(1).map(c => <option key={c}>{c}</option>)}</select></span></label>
    <label><CalendarDays size={19} /><span>Travel date<input type="date" value={date} onChange={e => setDate(e.target.value)} aria-label="Travel date" /></span></label>
    <button type="submit" className="btn btn-orange search-submit"><Search size={18} /> <span>Explore</span></button>
    <button type="button" className="mobile-search-link" onClick={() => onQuote(destination)}>Need a custom plan?</button>
  </form>;
}
